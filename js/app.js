...
function initGalleryPage() {
  ...
  console.log("Gallery ID obtenida:", galleryId);
  console.log('Gallery ID:', galleryId); // Depuración
...
function loadGalleryPhotos(galleryId) {
  ...
  // Load sub-galleries only if they exist
  const subGalleriesContainer = document.getElementById('subGalleriesContainer');
  
  if (subGalleriesContainer) {
    if (gallery.subGalleries && gallery.subGalleries.length > 0) {
      loadSubGalleries(gallery.subGalleries);
    } else {
      subGalleriesContainer.style.display = 'none';
    }
  }
}
...
function loadSubGalleries(subGalleries) {
  const subGalleriesContainer = document.getElementById('subGalleriesContainer');
  
  if (!subGalleriesContainer) return;
  
  subGalleriesContainer.innerHTML = '';
  
  subGalleries.forEach((subGallery, index) => {
    const subGalleryElement = document.createElement('div');
    subGalleryElement.className = 'sub-gallery-card';
    subGalleryElement.setAttribute('data-delay', index * 100);
    subGalleryElement.innerHTML = `
      <a href="galeria.html?id=${subGallery.id}" class="sub-gallery-card-inner">
        <div class="sub-gallery-card-image">
          <img src="${subGallery.coverImage}" alt="${currentLanguage === 'es' ? subGallery.title : subGallery.titleEn}">
        </div>
        <div class="sub-gallery-card-gradient"></div>
        <div class="sub-gallery-card-content">
          <h3 class="sub-gallery-card-title">${currentLanguage === 'es' ? subGallery.title : subGallery.titleEn}</h3>
          <p class="sub-gallery-card-description">${currentLanguage === 'es' ? subGallery.description : subGallery.descriptionEn}</p>
        </div>
      </a>
    `;
    subGalleriesContainer.appendChild(subGalleryElement);
  });
}
